...
$(document).ready(function() {
	...
	$("#subcategory").change(function(){
		$.ajax({
			url: '/listfoodsubcate',
			type: 'POST',
			data: {"page" : 1, "foodcate": myselect.options[myselect.selectedIndex].value},
			success: function(data){
				$("#foodsubcatetbl tbody tr").remove();
				if(data.result[0]){
...
function drawData(dataJson){
	...
	for(var i = 0; i< dataJson.length; i++){
		...
		tmpRow += '			<td></td>'
		tmpRow += '			<td><a href="#" class="foodsubcateid">' + dataJson[i].name + '</a></td>'
		tmpRow += '			<td><span class="label label-success">' + dataJson[i].code + '</span></td>'
		tmpRow += '			<td id="rowID" style="display:none;">' + dataJson[i]._id + '</td>';
		tmpRow += '			<td class="visible-md visible-lg">'
		tmpRow += '				<div class="btn-group">'
		tmpRow += '					<a title="" href="#" data-original-title="Edit" class="btn btn-sm show-tooltip foodsubcateid">'
		tmpRow += '					<i class="icon-edit"></i></a>'
		tmpRow += '					<a title="" href="#" data-original-title="Delete" class="btn btn-sm btn-danger show-tooltip delfoodsubcateid">'
		tmpRow += '					<i class="icon-trash"></i></a>'
		tmpRow += '				</div>'
		tmpRow += '			</td>'
		tmpRow += '		</tr>';

		$("#foodsubcatetbl tbody").append(tmpRow);
	}

	//-------------------------------
	// Event click on each item name foodsubcate
	//-------------------------------
	$(".foodsubcateid").click(function(){
		var foodsubcateid = $(this).closest('tr').find("#rowID").html();

		// Call ajax to get location
		var input = {"foodsubcateid" : foodsubcateid};
		$.ajax({
			url: '/admfoodsubcate',
			type: 'POST',
			data: input,
			success: function(data){
				if(data){
					window.location.href = '/foodsubcate';
				}
			},
			error: function(jqXHR){
				console.log(jqXHR.responseText+' :: '+jqXHR.statusText);
	...
	//-------------------------------
	$(".delfoodsubcateid").click(function(){
		var foodsubcateid = $(this).closest('tr').find("#rowID").html();
...
function splitPage(total){
	...
	$("#pagingid li a").click(function(){
		...
		curPage = idActive;
		var input = {"page" : idActive, "foodcate": 4};
		$.ajax({
			url: '/listfoodsubcate',
			type: 'POST',
			data: input,
