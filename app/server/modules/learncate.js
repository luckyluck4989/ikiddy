...
exports.addItem = function(input, callback){
	if(input.itemid == ''){
		...
		learncateDB.save(itemEntry, {safe: true}, callback);
	} else {
...
//--------------------------------
// Get list learn category
// Param callback: funtion callback
//--------------------------------
exports.getAllLearnCate = function(callback){
	learncateDB.find().sort([['name','asc']]).toArray(function(err,result){
		if(err)
...
// Param callback: funtion callback
//--------------------------------
exports.getLearnCategoryByID = function(cateid, callback){
	learncateDB.findOne({_id:new ObjectID(cateid)}, function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
// Param callback: funtion callback
//--------------------------------
exports.getListLearnCate = function(page, offset, callback){
	var iSkip = (page - 1) * offset;
	var iOffset = page * offset;
	learncateDB.find().sort([['name','asc']]).skip(iSkip).limit(iOffset).toArray(function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
//--------------------------------
// Get count list learn category
// Param callback: funtion callback
//--------------------------------
exports.getCountLearnCate = function(callback){
	learncateDB.count({}, function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
// Param callback: funtion callback
//--------------------------------
exports.deleteLearnCate = function(idcate, callback){
	learncateDB.remove({ _id : new ObjectID(idcate) }, function(err,result){
		if(err)
