...
exports.addItem = function(input, callback){
	if(input.itemid == ''){
		...
		videocateDB.save(itemEntry, {safe: true}, callback);
	} else {
		videocateDB.update( { _id : new ObjectID(input.itemid) }, 
							{ $set : { name 		: input.name,
									   image		: input.image
							} }, function(err,result){
			if(err)
...
// Param callback: funtion callback
//--------------------------------
exports.getAllVideoCate = function(callback){
	videocateDB.find().sort([['name','asc']]).toArray(function(err,result){
		if(err)
...
// Param callback: funtion callback
//--------------------------------
exports.getVideoCategoryByID = function(cateid, callback){
	videocateDB.findOne({_id:new ObjectID(cateid)}, function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
// Param callback: funtion callback
//--------------------------------
exports.getListVideoCate = function(page, offset, callback){
	var iSkip = (page - 1) * offset;
	var iOffset = page * offset;
	videocateDB.find().sort([['name','asc']]).skip(iSkip).limit(iOffset).toArray(function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
//--------------------------------
// Get count list video category
// Param callback: funtion callback
//--------------------------------
exports.getCountVideoCate = function(callback){
	videocateDB.count({}, function(err,result){
		if(err)
			callback(err,'Can not get list location');
		else
			callback(null,result);
...
// Param callback: funtion callback
//--------------------------------
exports.deleteVideoCate = function(idcate, callback){
	videocateDB.remove({ _id : new ObjectID(idcate) }, function(err,result){
		if(err)
